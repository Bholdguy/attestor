...
| `@convex-dev/static-hosting` serves SPA at `<deployment>.convex.site` | ☑ | **Deployed 2026-09-11** — `npx @convex-dev/static-hosting upload --build` (D-12) against the dev deployment. **`https://brazen-snail-826.convex.site`** serves `/` → 200 + correct `<title>` + the 241 KB bundle; SPA fallback (`/any/deep/path`) → 200; `/assets/*.js` → 200. Backed by the dev deployment (all 8 env vars + the 4 standing-evidence licenses). A separate `npm run deploy` to a *prod* deployment is available but would need env vars re-set. |
...
| AgentMail create-inbox + `messages/send` path, field names, attachment shape | ☑ | **Step 8 (2026-09-11)** — create verified live (D-14). Send: `POST /v0/inboxes/{inbox_id}/messages/send`, body `{to, cc?, subject, text, html, attachments[]}`, attachment `{filename, content_type, content(base64), content_disposition}`, response `{message_id, thread_id}`. **Real send verified live (reviewer-approved):** one synthetic-fixture conflict (license `jh7ey5j60bbq45p00rddfdyrpx8e5fey`, identity/wrong_person) → `sendAlert` → AgentMail 200, `message_id <010001a08da25e7f-…@email.amazonses.com>`, `thread_id e8295906-…`, `send_status:"sent"` to `bholdguyyy161@gmail.com`. Row + case left in the deployment as standing "AgentMail does real work" evidence (license frozen). **Second real send (reviewer-approved) is the representative status-flip case:** license `jh7cyrhp1p5e1hsz7x9rr4xyys8e5q9p`, case `jn728m3zngetsd6rp5wan4xs918e4w4f` type `status`, `snapshot_a` (Active) ≠ `snapshot_b` (Expired), reason `status: board now "Expired" vs last confirmed "Active"`, `message_id <010001a08da5838e-…@email.amazonses.com>`, `thread_id 13336a6f-…`, sent. Exactly-once + failure paths covered by `alert-once` / `agentmail-failure` (mocked). |
